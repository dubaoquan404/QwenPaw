    ...
    server: {
      ...
      proxy: {
        ...
        "/api": {
          ...
          changeOrigin: true,
          configure: (proxy) => {
            // Fix SSE streaming: prevent buffering for text/event-stream responses
            proxy.on("proxyRes", (_proxyRes, _req, res) => {
              const ct = (_proxyRes.headers["content-type"] as string) || "";
              if (ct.includes("text/event-stream")) {
                // Ensure TCP Nagle algorithm is disabled for immediate chunk forwarding
